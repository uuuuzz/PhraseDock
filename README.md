...
点击快捷按钮，将常用 AI 提示词追加到当前输入框的光标处。支持连续点击组合提示词，由你检查后发送。

同一仓库维护 macOS 与 Windows 版本。两端共用提示词菜单、配置和操作逻辑；系统输入分别由 Swift 与 C# 原生组件处理。

项目状态：macOS Apple Silicon 已重新完成构建、签名和通用目标识别检查，其他应用的实际粘贴仍需逐项验收。Windows x64 已实现输入适配并完成编译、纯逻辑测试及便携目录打包；真实 Codex 输入、桌面视觉与焦点体验尚待手动验收。详见 [验证记录](docs/validation.md) 和 [Windows 使用与验收](docs/windows.md)。
...
2. 首次使用，在中央按钮上点右键，选择「打开辅助功能设置」，然后在系统设置中启用 **AI Prompt Quick Appender**。若列表中没有它，用 `+` 添加正在运行的 `PhraseDock.app`。
3. 回到目标应用，点击标准文本框，确保看见输入光标。点击中央 `+` 展开提示词，再点击需要的键帽。
4. 中央按钮的绿色小圆点表示目标输入框就绪。只会插入文字，不会按回车发送。

- 启动时只显示中央 `+`；点击后提示词围绕中心展开，中央变为 `−`。再次点击即可全部收回。
- 点击提示词后菜单保持展开，便于连续组合；只有点击中央 `−` 才收回。
- 拖动中央按钮的外圈可移动浮窗，中心位置自动保存。
- 中央按钮右键菜单和菜单栏气泡图标提供编辑、刷新、测试、隐藏和退出。
- 「打开输入测试」会打开一个本地测试框，用于测试插入、选区替换、连续点击和换行。关闭该窗口后，恢复正常目标策略。
...
点击「编辑提示词配置」，修改 JSON 文件并保存，然后点击「重新加载提示词」。初次运行会从项目的 `config/phrases.json` 复制默认值到：
...
实际使用的是上面的用户配置，后续构建不会覆盖它。

应用显示名称使用中英文新名称；程序文件名、配置目录、应用标识和签名证书沿用项目内部名称 `PhraseDock`，便于继续使用已有配置与授权。
...
{
  "schemaVersion": 1,
  "target": { "macMode": "all", "macBundleIds": ["com.openai.codex"], "windowsExecutables": ["Codex.exe"], "windowsPackageFamilyNames": ["OpenAI.Codex_2p2nqsd0c76g0"] },
  "phrases": [
...
支持 1～12 个按钮，每个提示词最多 8000 个 UTF-16 单元。文本按原样插入，包含换行和首尾空白。`id` 必须唯一。错误配置不会覆盖上一次已加载的有效配置；启动时遇到错误会显示提示并临时使用默认提示词。

macOS 新安装默认使用 `macMode: "all"`，允许当前前台应用中由辅助功能识别的标准文本框。密码框、禁用控件、登录界面、安全认证进程和本应用自身会被拒绝；Canvas、自绘编辑器和部分富文本编辑器可能无法识别。若只希望允许指定 Mac 应用，可把 `macMode` 改为 `allowlist` 并配置 `macBundleIds`。旧 Mac 配置缺少 `macMode` 时仍按允许列表处理。

Windows 的目标策略没有变化。旧 Windows 0.1.0 配置可以直接使用；缺少 Windows 字段时只在内存中补充默认值，不重写用户文件。Windows 商店版 Codex 的实际窗口进程可能叫 `ChatGPT.exe`，通过系统报告的程序包系列名称 `OpenAI.Codex_2p2nqsd0c76g0` 识别；其他程序包或没有此身份的同名进程不会因此放行。非商店版本继续使用 `windowsExecutables` 文件名白名单，不填写路径或通配符。
...
在「隐私与安全性 → 辅助功能」中，用 `+` 重新选择你实际运行的 `PhraseDock.app`；如果旧条目未被更新，先选中旧条目并移除，再重新添加。系统可能要求本机登录验证。完成后退出并重新打开 AI Prompt Quick Appender。
...
该命令会撤销本应用自身的这项授权，需要重新在系统设置中允许；不要省略应用标识以免重置其他应用。本次本机故障已通过这一流程修复。
...
开发运行时进程名可能是 Electron；日常使用请使用打包应用并给 AI Prompt Quick Appender 授权。
...
```text
config/phrases.json          默认提示词，跨平台共用
...
- 浮窗不获取键盘焦点；发送前再次检查前台应用和输入框。macOS 默认允许任意前台应用中的标准文本区域、文本框和组合输入框；Windows 继续使用 exe 与程序包系列名称允许列表。
- macOS 会拒绝 PhraseDock 自身、登录和安全认证进程、密码框及禁用控件。特殊编辑器仍需逐项实测。
...
- 中文输入法仍在组词时，先完成候选词确认，再点击提示词。跨输入法的组合态兼容需要进一步实测。

## 放射式轻立体键帽

- 收起窗口为 72×72px，只显示 64px 中央按钮；展开后按提示词数量使用 344×284、440×340 或 560×420px 透明区域。
- 当前六个提示词顺时针分布在单圈椭圆上。配置允许的 1～12 个按钮均有对应布局，并检查不重叠、不越界。
- 键帽按下时下移 2px、缩小至 98%；即使快速点击也会保留至少 72ms 的按压反馈。菜单展开约 145ms，并按配置顺序以 8ms 间隔错开出现。
- 中央按钮的可点击直径为 56px，外围保留 4px 拖动环。快速连续点击提示词时按点击顺序排队执行，其他按钮不会整体禁用或变暗。
- 宿主窗口保持透明，空白区域通过 `setIgnoreMouseEvents` 允许鼠标落到后面的应用；中央按钮、拖动外圈和键帽仍可交互。
- 中心位置是窗口持久化单位。展开靠近屏幕边缘时只向屏幕内移动到足以完整显示的位置。

## License

AI Prompt Quick Appender 使用 [MIT License](LICENSE)。
