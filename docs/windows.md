...
Windows x64 的代码、编译和便携目录打包已完成。0.1.1 加入实际前台身份的只读诊断；0.1.3 修复了初始空剪贴板序列号为 0 时被误拒绝的问题。此次排查只读取了实际剪贴板的状态元数据，并在独立窗口站检查了空剪贴板；未读取或写入用户剪贴板内容，未操控鼠标/键盘或切换目标应用。真实粘贴与剪贴板恢复尚未验收，详见 `validation.md`。

## 启动与更新

- 先退出旧版，再双击 `dist/0.1.3/PhraseDock-win32-x64/PhraseDock.exe`，保留整个目录。可自行创建该 exe 的桌面快捷方式。应用使用单实例锁，旧版仍运行时打开新版会唤起旧版。
...
- 用户提示词位于 `%APPDATA%\PhraseDock\phrases.json`，窗口位置位于同目录的 `window.json`。更换程序目录或重建不会覆盖它们。
- 0.1.2 的应用标题、托盘、菜单、测试页和程序属性采用中英文新名称；内部程序文件名和配置目录继续使用 `PhraseDock`，改名后仍读取原来的个人预设。
- JSON 缺少 Windows 字段时在内存中补入 `Codex.exe` 及 Codex 商店包身份；保留已有提示词和用户文件。非商店目标使用 `target.windowsExecutables`，商店包使用 `target.windowsPackageFamilyNames`；后者设为空数组可明确关闭商店包默认匹配。
...
初始空剪贴板的序列号可以是 0；0.1.3 通过打开剪贴板和格式枚举的实际结果判断可访问性。写入后仍记录序列号，并在恢复锁内比较，以保留用户后来复制的内容。
...
| 点击提示词、中央按钮、拖动外圈、右键菜单 | 原输入框焦点保留，按需手动关闭菜单后继续输入 |
...
Node.js 22.12+、PowerShell 7、.NET 10 SDK。运行 `npm ci`、`npm run package:win`。打包程序带 .NET 运行时，终端用户无需安装 SDK。`npm run build:native` 后可运行 `npm run test:windows`；包含使用实际 `ClipboardLease.cs` 配合内存原生接口替身的回归测试，以及原生组件的纯逻辑/协议自测，不访问实际桌面。打包过程也执行这些检查。
